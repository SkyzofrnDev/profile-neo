...
const ReviewCard = ({ review, index, hoveredCard, setHoveredCard }) => {
  return (
    <div
      className={`shrink-0 w-56 lg:w-96 bg-white rounded-2xl shadow-lg p-8 flex flex-col relative transition-all duration-300 cursor-pointer ${
        hoveredCard === index
          ? "scale-105 shadow-2xl border border-primary z-30"
    ...
    >
      ...
      <div className="absolute top-0 left-2 pointer-events-none">
        <span
          className="m-3 text-sm lg:text-6xl text-primary jakarta font-bold leading-none block"
        >
      ...
      {/* Konten review */}
      <p className="md:text-lg text-sm sm:text-base  mb-6 mt-2 leading-relaxed pl-2 relative z-10">
        {review.review}
      </p>

      {/* Profil */}
      <div className="flex text-sm sm:text-default items-left gap-4 mt-auto pt-4 border-t border-gray-200">
        {/* <img
          src={review.image}
          alt={review.name}
          className="w-7 h-7 lg:w-14 lg:h-14 rounded-full object-cover border-2 border-primary"
        /> */}
        <div>
          <p className="font-semibold  lg:text-lg">
            {review.name}
          </p>
          <p className="text-gray-600 lg:text-sm">{review.company}</p>
        </div>
      </div>
    </div>
  );
...
const Ratings = () => {
  ...
  return (
    <div className="text-center px-2 mt-12 lg:my-56 w-full max-w-full lg:px-24 flex flex-col items-center">
      <p className="text-3xl font-semibold text-default lg:text-5xl lg:mb-20">
        Apa Kata Klien Kami
      </p>
      <div
        className="text-left w-full relative py-12 flex flex-col gap-12"
        style={{ overflowX: "hidden", overflowY: "visible" }}
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <div
          className="absolute -left-1 lg:left-0 top-0 bottom-0 w-5 lg:w-48 z-20 pointer-events-none"
          style={{
            background:
        ...
        />
        <div
          className="absolute -right-1 lg:right-0 top-0 bottom-0 w-5 lg:w-48 z-20 pointer-events-none"
          style={{
            background:
